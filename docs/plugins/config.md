...
### Hotkey plugins
Hotkey plugins will only be executed, if the specified hotkey is pressed. They also provide the ability to ask the user for a specific `INPUT`, which then can be used as a variable while executing commands. The function keys <kbd>F9</kbd> to <kbd>F24</kbd> are reserved for hotkey plugins.
